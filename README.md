...
back when it dies (and goes back for what it dropped) and when the server
restarts; its settings have an in-game menu; most of what a player does is still
to come.
...
box, tell how it is, look around, tell where and how it last died. When
something it was asked is over (done, or given up), it says so, in its words.
Each player can speak to bots a few times a minute (`per_minute`).
...
others (`last_death`, and most of those still to come).
