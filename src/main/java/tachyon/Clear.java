 ...
 * it carries and the time the block takes with it.
 *
 ...
 * does, while there is room. Several bots told to clear the same box share it: each
...
final class Clear extends Job {
    ...
    /** The box, shared by every bot told to clear it together. */
    static final class Area {
        ...
        final Map<BlockPos, Bots.Bot> claimed = new HashMap<>();
         ...
         */
        BlockPos next(Bots.Bot p, Set<BlockPos> hidden) {
            // A claim of a bot no longer at it is free again.
         ...
         */
        boolean claim(BlockPos pos, Bots.Bot p) {
        ...
        void release(BlockPos pos, Bots.Bot p) {
            if (pos != null && claimed.get(pos) == p) claimed.remove(pos);
        }

        void giveUp(BlockPos pos, Bots.Bot p) {
            release(pos, p);
            given.add(pos);
        }

        private boolean working(Bots.Bot q) {
    ...
    boolean think(Bots.Bot p, long now) {
        if (p.body.level() != area.level) {
            Bots.halt(p, "the box to clear is in another dimension");
            return false;
        }
        ...
        if (goal == null) {
            ...
            if (goal == null) {
                if (area.done) {
                    Bots.halt(p, "cleared " + area.box() + ": " + broken + " broken by it, " + area.broken
        ...
        if (hit != null) {
            ...
            // another bot's): another block for now, this one later.
        ...
        p.plannedAt = now;
        BlockPos to = goal;
        Bots.plan(p, to, world -> reachOf(to), doing());
        return true;
    }
