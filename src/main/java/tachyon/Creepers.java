 ...
 *     arrows. Not from water, not with a player on or near the line, not at one swelling
 *     within 7, not at one with a name (someone cares for it: it only runs from that one),
 *     and not while something else is hitting it: with a zombie biting and a creeper in
 *     range, a Masurium bot that shot the creeper was bitten to death.</li>
 ...
 * within {@link #CLOSE 4} (round a corner it blows all the same). Not one that has no way
 * on foot to it (looked for on a routes thread, from the creeper to the bot, and taken
 * for a way while the answer is due: when in doubt, it is danger), for 30 s; nor one that
 * made it run three times in 90 s without reaching it (a ledge, water, a one-block gap), for
 * 90 s: with two or three of those around, a Masurium bot did nothing else all night. Never
 * one within {@link #EXPLOSION 7}: the rules are for the far scare, never the one on top.
 *
...
final class Creepers implements Ability {
    ...
    static final class Known {
        ...
        private final Map<UUID, ArrayDeque<Long>> scares = new HashMap<>();
        ...
        /**
         * One more scare by the creeper {@code id}, at tick {@code now}: whether it is the
         * {@link #SCARES_MAX third} within {@link #SCARES_WINDOW 90 s}; then that one is left
         * alone for 90 s, and its count starts again.
         */
        boolean scared(UUID id, long now) {
            ArrayDeque<Long> q = scares.computeIfAbsent(id, k -> new ArrayDeque<>());
            while (!q.isEmpty() && now - q.peekFirst() > SCARES_WINDOW) q.removeFirst();
            q.addLast(now);
        ...
        void forget(long now) {
            ...
            scares.values().removeIf(q -> q.isEmpty() || now - q.peekLast() > SCARES_WINDOW);
        }
    }
    ...
    public void settings(Settings settings) {
        settings.bool(SHOOT, true, "whether it shoots creepers 10 to 25 blocks away with its bow by itself; without it,"
                ...
                .label("Shoot creepers").group("Fighting").advanced();
    }
    ...
    public void tick(Bots.Bot p, long now) {
        ...
        if (look || c == null || !c.isAlive() || c.level() != b.level()) c = choose(p, now);
        if (c == null) {
        ...
        boolean shot = Settings.bool(p, SHOOT) && d >= BOW_MIN && d <= VIEW && !(swelling && d <= EXPLOSION)
                ...
                && !b.isInWater() && !Defending.fighting(p) && Bow.missing(b) == null
                && b.hasLineOfSight(c) && !Bow.someoneInTheLine(b, c);
        ...
        if (run) {
            ...
            // A new scare. The third from one creeper in 90 s without it reaching: it cannot
     ...
     * 4), and neither left alone (no way to it, too many scares) nor without a way on foot to
     * it; within {@link #EXPLOSION} it counts whatever the rules say.
     */
    private static Creeper choose(Bots.Bot p, long now) {
        BotPlayer b = p.body;
        Creeper best = null;
     ...
     * the bot (a fall of 8 at most, 3000 tiles: Masurium's), asked again after 2 s. Due, it
     * counts as a way. None, the creeper is left alone for 30 s.
     */
        ...
        if (r != null) {
            ...
            if (!answer(r)) {
                ...
                LOG.info("[tachyon] {} leaves alone a creeper {} away: it has no way on foot to it (for {} s)",
                        p.name(), Threats.blocks(p.body.distanceTo(c)), NO_WAY_TICKS / 20);
                return false;
            }
            if (now < r.until()) return true;
        }
        BlockPos from = c.blockPosition(), to = p.body.blockPosition();
        known.reach.put(c.getUUID(), new Reach(Bots.search(p.body.serverLevel(), from, to, world -> way(world, from, to)),
                now + REACH_KEPT));
        return true;
    }
    ...
    /** One more scare by {@code c}: whether it is the third within 90 s, and then it is left alone for 90 s. */
    private static boolean scaredTooOften(Bots.Bot p, Known known, Creeper c, long now) {
        if (!known.scared(c.getUUID(), now)) return false;
        LOG.info("[tachyon] {} leaves alone a creeper {} away: {} scares in {} s without it coming (for {} s)",
                p.name(), Threats.blocks(p.body.distanceTo(c)), SCARES_MAX, SCARES_WINDOW / 20, SCARED_TICKS / 20);
        return true;
    }
}
