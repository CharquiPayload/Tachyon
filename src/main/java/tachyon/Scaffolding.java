...
final class Scaffolding implements Ability {
    ...
    public void settings(Settings settings) {
        settings.bool(BUILD, true, "When there is no way on foot, it places blocks it carries (dirt, stone, planks...)"
                        + " to get where it goes: a bridge over a gap, a tower to climb.", Settings.Who.OWNER)
                .label("Build to get there").group("Walking").basic();
        settings.bool(WHILE_FOLLOWING, true, "When there is no way on foot, it also builds bridges and towers with"
                        + " blocks it carries to keep up with whom it follows.", Settings.Who.OWNER)
